      ...
      fetch(
        ...
        .then((res) => {
          if (res.status) {
            ...
            console.error("天气信息可能暂时不支持哦");
            iziToast.show({
              timeout: 2000,
              icon: "fa-solid fa-cloud-sun",
                message: "天气信息可能暂时不支持哦",
            });
          }
        });
    })
    .catch((err) => {
        console.error("天气信息可能暂时不支持哦：" + err);
      iziToast.show({
        timeout: 2000,
        icon: "fa-solid fa-cloud-sun",
          message: "天气信息可能暂时不支持哦",
...
$("#github")
  .mouseover(function () {
    $("#link-text").html("去油管看看？");
  })
  .mouseout(function () {
      $("#link-text").html("看看我的联系方式和社交网络?");
  });
$("#qq")
  .mouseover(function () {
      $("#link-text").html("目前还没有哦~");
  })
  .mouseout(function () {
    $("#link-text").html("看看我的联系方式和社交网络?");
  });
...
$("#switchmore").on("click", function () {
  ...
  if (shoemore && $(document).width() >= 990) {
    $("#container").attr("class", "container mores");
      $("#change").html("Qwarz&nbsp;!");
    $("#change1").html("哦呀?（ 再点击一次可关闭 ）");
  } else {
    $("#container").attr("class", "container");
      $("#change").html("Qwarz&nbsp;.Atarz&nbsp;!");
      $("#change1").html("绝对！不是！baka！  |  bilibili:Qwarz_Atarz");
  }
});
